...
mpi__det(double *matrix, size_t len, size_t threads, int rank)
{
    ...
    if (rank < working_threads) {
        assigned_rows = len / working_threads + (rank < len % working_threads);
    } else {
        assigned_rows = 0;
    }
    ...
    for (int diag_idx = 0; diag_idx < len; ++diag_idx) {
        ...
        MPI_Bcast(diag_row + non_zeros, curr_row_len, MPI_DOUBLE, diag_assigned_working_rank, MPI_COMM_WORLD);
